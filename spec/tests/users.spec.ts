...
describe('UserRouter', () => {
  ...
  describe(`"POST:${Paths.Users.Add}"`, () => {
    ...
    const callApi = (user: IUser | null, cb: TApiCb) =>
      agent.post(Paths.Users.Add).send({ user }).end(apiCb(cb))
  ...
  describe(`"PUT:${Paths.Users.Update}"`, () => {
    ...
    // Setup API
    const callApi = (user: IUser | null, cb: TApiCb) =>
      agent.put(Paths.Users.Update).send({ user }).end(apiCb(cb))
  ...
  describe(`"DELETE:${Paths.Users.Delete}"`, () => {
    ...
    const callApi = (id: number, cb: TApiCb) =>
      agent.delete(insertUrlParams(Paths.Users.Delete, { id })).end(apiCb(cb))
